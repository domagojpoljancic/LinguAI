...
- **Topic identification:** Its own node `topic_identification` after the level router. Deterministic keyword matching against a small map (e.g. “restaurant”, “travel”, “business”) to produce a label like “restaurant vocabulary”. Stored in state `topic` for the box-creation step. Hybrid: deterministic first; AI fallback when general/low confidence. State: topic, topic_confidence, topic_source, topic_reason. Response: topicSource, topicConfidence, topicReason. Situation-aware ranking when topic from AI.
