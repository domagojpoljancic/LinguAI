...
def _primary_and_widen_topics(display_topic: str | None) -> Tuple[str, List[str]]:
    ...
    Widening is ordered from closest to more generic topics.
    """
    ...
    widening_map: Dict[str, List[str]] = {
        "restaurant": ["restaurant", "daily", "travel"],
        "travel": ["travel", "daily"],
        "business": ["business", "daily"],
        "shopping": ["shopping", "daily"],
        "health": ["health", "daily"],
        "dating": ["dating", "daily"],
        "daily": ["daily", "general"],
...
def retrieve_candidates(
    ...
    *,
    ...
    def consider_rows(rows: Sequence[sqlite3.Row]) -> None:
        ...
        for row in rows:
            ...
            if learner_rank and _level_rank(lvl) > learner_rank:
                # Skip harder than learner level when we know the level.
                continue
    ...
    if len(all_selected) < max_items and widened_topics:
        ...
        # Sort widened by (score desc, level_rank asc) to prefer useful + easier
        widened_rows_sorted = sorted(
            widened_rows,
            key=lambda r: (-float(r["score"] or 0.0), _level_rank(r["level"]), (r["default_text"] or "")),
        )
        consider_rows(widened_rows_sorted)

    # Sort final selection: score desc, level_rank asc, default_text asc
    final_rows = sorted(
        all_selected,
        key=lambda r: (-float(r["score"] or 0.0), _level_rank(r["level"]), (r["default_text"] or "")),
    )

    words = [{"default": r["default_text"], "target": r["target_text"]} for r in final_rows]
