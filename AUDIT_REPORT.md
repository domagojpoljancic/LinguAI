...
POST /generate-boxes
  ...
  → graph.invoke(initial)  [LangGraph]
       ...
       → topic_identification (node: deterministic keywords or AI classifier fallback when topic is general/low confidence)
...
| **topic_identification** | Yes | Yes (deterministic vs AI) | topic, topic_source, topic_confidence, topic_reason | Yes | N/A |
| **level_resolution** | Yes | Yes (explicit vs inferred, level value) | level, level_source | Yes | Yes (in _infer_level_with_llm) |
| **box_creation_placeholder** | Yes | N/A (deterministic retrieval; situation-aware ranking when topic from AI) | — | Yes (“reached”) | N/A |
